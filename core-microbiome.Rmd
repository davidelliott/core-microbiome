...
core_microbiome <- function(physeq,threshold=2,method="mean",ranks=c("Species"),verbose=TRUE){
  core_otus_glom <- c()
  for (rank in ranks) {
    ...
    if(verbose){
      print(paste(rank,"level search found",length(glom_rank_candidates),"taxa above",method,"abundance of",threshold))
      print(paste("Search found", length(core_otus_glom),"unique OTUs"))
      }
  }
...
# Input 2 phyloseq objects
# returns nothing
# prints a list of extra OTUs in the first object
# Identifies extra taxa represented by those OTUs, which are not present in the second object
...
for(this_expt in  expt_list) {
    plot_title <- paste(expt_names[n],"-",ntaxa(this_expt),"otus")
    print(plot_bar(this_expt, fill="Phylum",title=plot_title))
    expt_names[1]
    n <- n+1
}
