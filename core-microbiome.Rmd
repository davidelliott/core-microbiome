...
# Make a merged object for bacteria
( all.bac.rel.original <- merge_phyloseq(hb.rel,kb.rel) )

# When merging initially the number of taxa will be the sum of the merged objects
# So tax_glom can potentially finish the merging process to combine similar OTUS
# however care needs to be taken because missing taxonomic information can cause
# different OTUs to be merged together
# Note the phyloseq documentation for tax_glom

( all.bac.rel <- tax_glom(all.bac.rel.original, "Species",NArm = FALSE) )
...
expts <- c(kb.rel , hb.rel, all.bac.rel , hf.rel)
studies <- c("dryland bacteria" , "peatland bacteria" , "all_bacteria", "peatland fungi")
